...
struct node_t {
	...
	union {
		...
		struct node_t*	next;  // Directive / Loop
		char*			str;   // String
	};
...
extern struct node_t* create_loop(struct node_t* parent, struct node_t* start, struct node_t* next);
...
extern struct node_t* apply_directive(struct node_t* node, struct node_t* directive_node, bool* loop, struct array_t* mac_array, struct array_t* str_array);
