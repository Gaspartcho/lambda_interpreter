...
char get_parser_next_char(struct parser_t* parser) {
	...
	if (parser->next_char == COMMENT_OPEN) {
		...
		return get_parser_next_char(parser);
	}
...
void parse_name(struct parser_t* parser, name_t name, bool (*pattern_func)(char)) {
	...
	for (i = 0; i < NAME_MAX && pattern_func(parser->next_char); i++) {
...
struct node_t* parse_var(struct parser_t* parser, struct array_t* var_array) {
	...
	if (strncmp(name, "", NAME_MAX) == 0) error(E_EPT_TOK_NAME);
...
struct node_t* parse_function(struct parser_t* parser, struct array_t* var_array, struct array_t* mac_array) {
	...
	do {
		...
		if (strncmp(token->name, "", NAME_MAX) == 0) error(E_EPT_TOK_NAME);

		add_array_elem(var_array, token);

		b_func->body = create_function(b_func, NULL);
		b_func		 = b_func->body;
	} while (parser->next_char == COMMA);

	if (parser->next_char != DOT) error_c(E_INV_TOK, parser->next_char);
...
struct node_t* parse_application(struct parser_t* parser, struct array_t* var_array, struct array_t* mac_array) {
	...
	if (parser->next_char != APPLY_CLOSE) error_c(E_INV_TOK, parser->next_char);
...
struct node_t* parse_macro(struct parser_t* parser, struct array_t* mac_array) {
	...
	if (!strncmp(name, "", NAME_MAX)) error(E_EPT_TOK_NAME);
...
struct node_t* parse_directive(struct parser_t* parser) {
	...
	char l_char = parser->next_char;
	get_parser_next_char(parser);

	switch (l_char) {
		case DISPLAY:	return create_directive(NULL, display, d_Display); break;
		case TREE:		return create_directive(NULL, tree, d_Tree); break;
		case EVAL:		return create_directive(NULL, evaluate, d_Eval); break;
		case EVAL_ONCE: return create_directive(NULL, eval_once, d_Evonce); break;
		case MACRO:		return create_directive(NULL, macro, d_Macro); break;
		default:		error_c(E_INV_TOK, l_char); return NULL;
	}
}
